...
// Setting the variable 'owner' to the address of the contract deployer. 
    ...
    constructor() public { 
        // When the code is deployed, assign owner to the person who deployed the smart contract
        // the person who deployed is gotten inside of msg.sender
    owner = msg.sender;
    } 
...
modifier marksChecking(uint inputMark){
    
        require(inputMark > 0 && inputMark <= 100, "Marks should not be above 100");
        _;
    } 
...
// into one user defined type. 
    ...
    function adding_values(address _address, string memory _name, string memory _subject, uint8 _mark) 
    public marksChecking(_mark) { 
